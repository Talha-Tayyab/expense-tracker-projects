class ExpenseTracker {
    ...
    createCategoryFilters() {
        const categories = [
            { id: 'food', emoji: '🍽️', name: 'Food & Dining' },
            { id: 'transport', emoji: '🚗', name: 'Transportation' },
            { id: 'shopping', emoji: '🛍️', name: 'Shopping' },
            { id: 'entertainment', emoji: '🎬', name: 'Entertainment' },
            { id: 'bills', emoji: '📄', name: 'Bills & Utilities' },
            { id: 'health', emoji: '🏥', name: 'Healthcare' },
  ...
  // Update filters to include all current categories
  ...
  `).join('');
    ...
    applyFilters() {
        this.filteredExpenses = this.expenses.filter(expense => {
            ...
            if (this.filters.categories.length > 0 && !this.filters.categories.includes(expense.category)) {
                return false;
            }

            if (this.filters.dateFrom && expense.date < this.filters.dateFrom) return false;
            if (this.filters.dateTo && expense.date > this.filters.dateTo) return false;

            if (expense.amount < this.filters.minAmount || expense.amount > this.filters.maxAmount) return false;
    ...
    updateFilterCount() {
        ...
        if (filtered === total) {
            filterCount.textContent = `Showing all ${total} expenses`;
        } else {
            filterCount.textContent = `Showing ${filtered} of ${total} expenses`;
        }
    }
        ...
        switch(fieldId) {
            case 'amount':
                ...
                if (!field.value || amount <= 0 || amount > 1000000) {
                    this.showError(field, errorElement, 'Please enter a valid amount (0.01 - 1,000,000)');
                    isValid = false;
                }
                break;
            case 'category':
            ...
            case 'date':
                ...
                if (!field.value || selectedDate > today || selectedDate < oneYearAgo) {
                    this.showError(field, errorElement, 'Please select a valid date (within the last year)');
                    isValid = false;
                }
                break;
            case 'description':
                if (field.value.length > 200) {
                    this.showError(field, errorElement, 'Description must be less than 200 characters');
                    isValid = false;
                }
                break;
        }
            ...
            return `
                <div class="expense-item">
                    ...
                    <div class="expense-actions">
                        <button class="btn btn-danger btn-small" onclick="app.deleteExpense('${expense.id}')">
                            Delete
                        </button>
                    </div>
                </div>
...
getCategoryEmoji(categoryId) {
  ...
  return cat ? cat.emoji : '📦';
    ...
    resetForm() {
        ...
        this.setDefaultDate();
        ['amount', 'category', 'date', 'description'].forEach(id => this.clearError(id));
    }
...
// Step 3: Budget Management System (extends ExpenseTracker)
class BudgetTracker extends ExpenseTracker {
    ...
    setupBudgetUI() {
        // Add a new card below summary for budget controls
        ...
        document.querySelector('.container').insertBefore(
            budgetCard, 
            document.querySelector('.card.expense-list')
        );
    ...
    handleBudgetSubmit(e) {
        ...
        this.saveBudgets();
        this.updateBudgetUI();
        document.getElementById('budgetFeedback').textContent = 'Budget saved successfully ✅';
    }
...
`;
        ...
        if (!currentBudget) {
            barsContainer.innerHTML = '<p style="color:#7f8c8d">No budget set for this month</p>';
            return;
        }
        ...
        this.getAllCategories().forEach(cat => {
            ...
            barsContainer.innerHTML += `
                <div style="margin-bottom:10px;">
                    <div style="margin-bottom:4px; font-weight:600;">${this.getCategoryEmoji(cat)} ${this.getCategoryName(cat)}: $${spent.toFixed(2)} / $${budget.toFixed(2)}</div>
        ...
        // Show total budget comparison
        const totalSpent = this.expenses
            .filter(e => e.date.startsWith(month))
            .reduce((sum, e) => sum + e.amount, 0);
        const totalBudget = currentBudget.total || 0;
        let totalPercent = totalBudget > 0 ? Math.min((totalSpent / totalBudget) * 100, 100) : 0;
        ...
        barsContainer.innerHTML += `
            <div style="margin-top:16px;">
                <div style="margin-bottom:4px; font-weight:700;">Total: $${totalSpent.toFixed(2)} / $${totalBudget.toFixed(2)}</div>
    ...
    // Update budget bars when new expenses are added/deleted
    addExpense(expense) {
...
// This extends the BudgetTracker class to include charts using Chart.js

class ChartedTracker extends BudgetTracker {
    constructor() {
        super();
        this.chartInstances = {}; // to keep track of chart objects
        this.setupChartUI();
        this.updateCharts();
    }
    ...
    renderCategoryPieChart() {
        ...
        const labels = Object.keys(data).map(cat => `${this.getCategoryEmoji(cat)} ${this.getCategoryName(cat)}`);
        ...
        this.chartInstances.pie = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    data: values,
                    backgroundColor: [
                        '#3498db', '#2ecc71', '#f1c40f', '#e67e22', '#e74c3c', '#9b59b6', '#95a5a6'
                    ]
                }]
            },
            options: {
...
// CategoryManager with Edit & Delete + Reassignment (Phase 4)
...
class CategoryManager extends ChartedTracker {
  ...
  constructor() {
    ...
    this.refreshBudgetCategories();      // 👈 updates budget input boxes
    this.createCategoryFilters();        // 👈 updates filter checkboxes
    this.setupGlobalDeleteButton();  // 👈 New method call
    ...
    this.categories.forEach(cat => {
      ...
      box.innerHTML = `
        ...
        <div class="category-actions">
          ...
          <button class="btn btn-small btn-success save-btn" data-id="${cat.id}" style="display:none">Save</button>
      ...
      deleteBtn.addEventListener('click', () => {
        if (cat.expenseCount > 0) {
          const reassignTo = prompt(`Category '${cat.name}' has ${cat.expenseCount} expenses. Enter ID of a category to reassign to:`);
 ...
 updateCategoryDropdown() {
    ...
    // Ensure the empty value is selected initially
    select.value = "";
}
  ...
  setupCategoryForm() {
    ...
    form.addEventListener('submit', (e) => {
      ...
      this.renderCategoryGrid();
      this.updateCategoryDropdown();
      this.refreshBudgetCategories();  // ✅ Budget section updates
  this.createCategoryFilters();    // ✅ Filters update
      modal.style.display = 'none';
    });
  ...
  deleteBtn.addEventListener('click', () => {
    ...
    if (expenseCount > 0) {
      const reassignTo = prompt(`Category '${cat.name}' has ${expenseCount} expenses. Enter name or ID of a category to reassign to:`);

      const newCat = this.categories.find(c =>
...
class ImportExportManager extends CategoryManager {
  constructor() {
    super();
    this.setupImportExportUI(); // 👈 adds buttons for JSON import/export
  }
  ...
  // Placeholder methods (we’ll implement these next)
 readJSONFile(file) {
...
exportToJSON() {
  ...
  this.downloadFile(jsonString, `expenses_${new Date().toISOString()
    .split('T')[0]}.json`, 'application/json');
}
